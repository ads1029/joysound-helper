...
- `joysound-priority-ranked-2000-next-expansion-candidates.json`：第三轮一阶段来源扩展候选索引，加入官方热门特集、当前歌手榜和前 1,000 首歌手热门曲后共 10,087 个页面，其中 4,486 页尚未进入生产曲库；仍需继续扩展到 20,000 页并采集详情。
- `joysound-priority-ranked-2000-next-expansion-audit.json`：上述 4,486 个生产未收录页面的阶段审计报告；详情采集尚未开始时覆盖 0/4,486，待处理 4,486。
- `joysound-priority-ranked-2000-next-feature-candidates.json`：第三轮完成来源扩展后的 20,000 个候选页面；原始索引有 15,991 页尚未进入生产曲库，年份范围 2016～2026，与前两轮候选零重叠；首批晋级后当前剩余 13,481 页待详情采集。
- `joysound-priority-ranked-2000-next-songs.json`：第三轮首批 3,086 页详情结果，成功 3,000、无 X1 85、不可用 1、错误 0。
- `joysound-priority-ranked-2000-next-audit.json`：第三轮首批详情严格审计结果，覆盖 3,086/3,086，冲突 0，合并曲库 34,135 首、44,601 个版本。
- `joysound-priority-ranked-2000-next-review-sample.json`：第三轮首批详情的 20 首低速来源复核报告，20/20 通过。
- `joysound-priority-ranked-2000-next-feature-pending-candidates.json`：以第三轮 20,000 页索引和已晋级生产曲库为基线重新固化的 13,481 个待采集详情页面，年份范围 2020～2026，与前两轮候选零重叠。
- `joysound-priority-ranked-2000-next-feature-pending-songs.json`：上述 13,481 个待采集页面的进行中详情结果，使用独立检查点；最新检查点已处理 1,618 页，成功 1,401、无 X1 216、不可用 1、错误 0，完成后再审计和复核。
- `joysound-production-catalog.json`：通过生产晋级门禁后生成的精简前端曲库；当前包含 34,135 首歌曲和 44,601 个版本，不包含待处理 URL、错误明细等审计状态。
...
两轮优先榜单批次均已完成：首轮 5,127/5,127 页、第二轮 3,054/3,054 页覆盖，第二轮成功 2,824、无 X1 230、错误和冲突均为 0；第二轮 20 首来源复核通过，当前复核输入指纹与生成歌曲一致，完整轮次已晋级为 31,135 首歌曲、41,171 个版本。第三轮已固化 20,000 个年份过滤候选；首批 3,086 页已完成采集、审计和 20 首复核，成功 3,000、无 X1 85、不可用 1、错误 0，新增 3,000 首并晋级，生产曲库现为 34,135 首、44,601 个版本；剩余 13,481 页已切分为独立待采集索引，最新检查点已处理 1,618 页，成功 1,401、无 X1 216、不可用 1、错误 0，剩余 11,863 页，后续从检查点恢复并完成严格审计、复核和生产晋级。全曲目详情长期任务仍剩余 300 页，下次从 `offset=17987` 谨慎继续。
